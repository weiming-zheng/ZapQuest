...
@Component
@Slf4j
public class JwtTokenChildInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        try {
            ...
            Long userId = Long.valueOf(claims.get(JwtClaimsConstant.CHILD_ID).toString());
            log.info("当前用户的id：", userId);
            BaseContext.setCurrentUser(JwtClaimsConstant.CHILD_ID, userId);
            //3、通过，放行
            return true;
        } catch (Exception ex) {
            //4、不通过，响应401状态码
